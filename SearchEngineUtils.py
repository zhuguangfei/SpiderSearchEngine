...
def baidu(word, start=0, end=50):
    ...
    index = 1
    for url in urls:
        if not os.path.exists(os.path.join(package, f'{index}_v.html')):
            if 'baidu.php' in url:
                # rspTemp = requests.get(url=url)
                # with open(os.path.join(package, f'{index}_v.html'), 'w', encoding="utf-8") as w:
                ...
                print(os.path.join(
                    package, f'{index}_v.html'))
            elif 'link' in url:
                with open('temp.html') as r:
                    template = r.read()
                with open(os.path.join(package, f'{index}.html'), 'w', encoding="utf-8") as w:
                    w.write(template % (url, url))
                print(os.path.join(package, f'{index}.html'))
        index = index+1
